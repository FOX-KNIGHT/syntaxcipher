...
// ─── 5. ROW TRANSPOSITION ─────────────────────────────────────────────────────
export function rowTransposition(text, key, decrypt = false) {
  const t = text.toUpperCase().replace(/[^A-Z]/g, "");
...
export function hill(text, keyMatrix, decrypt = false) {
  const t = text.toUpperCase().replace(/[^A-Z]/g, "");
...
export function doubleTransposition(text, key1, key2, decrypt = false) {
  if (!decrypt) {
...
export function applyCipher(cipherName, text, params, decrypt = false) {
  if(!params) params = {};
